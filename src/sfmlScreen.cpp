...
namespace yn
{
    ...
    {
        m_vertices.resize(width * height * 6); //each quad have 4 vertices
        m_width = width;
        m_height = height;
        ...
        for (std::size_t x = 0; x < width; ++x)
            for (auto y = 0; y < height; ++y)
            {
                ...
                //top-left
                m_vertices[index + 4].position = coord2d;
                m_vertices[index + 4].color = sfcolor;

                //bottom-right
                m_vertices[index + 5].position = coord2d + sf::Vector2f{m_pixelSize, m_pixelSize};
                m_vertices[index + 5].color = sfcolor;
            }
    }

    void SFMLScreen::setPixel(std::size_t x, std::size_t y, uint32_t color)
    {
        auto index = (x * m_height + y) * 6;
